...
def overlap_of_reshaped_sets(vectors_set_0, vectors_set_1):
    overlaps = []
    # We want to iterate over as many elements, as there are in the second set
    # (that's why we are using np.shape(vectors_set_1))!
    # This method might be used with case, in which we are reproducing input of the neural net,
    # so vectors_set_1 might be #_of_timesteps smaller, than vectors_set_0.
    for i in range(np.shape(vectors_set_1)[0]):
...
def energies_of_reshaped_set(vectors_set, hamil):
    ...
    for vector in vectors_set:
        ...
        energy = np.dot(compressed_vector, np.dot(hamil, compressed_vector.T))[0][0]
