...
for N in range(N_min, N_max+1):
    ...
    # print("(1) Eigenvecs: ", np.shape(eigenvectors))
    reshaped_eigenvectors = generators.reshape_set(eigenvectors)
    # print("(2) Eigenvecs: ", np.shape(eigenvectors))
    reshaped_test_input_vectors = generators.flatten_set(reshaped_eigenvectors)
    # print("(3) Eigenvecs: ", np.shape(reshaped_test_input_vectors))
    # sys.exit()
    ...
    for time_step in x:
        ...
        for pair in pairs:
            ...
            print("correlations_full_op_real: ", correlations_full_op_real)

            if (N0,N1) not in y_mean_correlations:
                y_mean_correlations[N0, N1] = {"real":[], "imag":[]}
            # Instead of adding the mean of correlations add the correlation in just the first evolved vector
            y_mean_correlations[N0, N1]["real"].append([correlations_full_op_real[0], correlations_nn_real[0]])
            y_mean_correlations[N0, N1]["imag"].append([correlations_full_op_imag[0], correlations_nn_imag[0]])
