...
def generate_training_set(training_set_size, vector_dims, H, t):
    print("##################################################")
    print("####### Generating training set - (many timesteps)")
    print("##################################################")
...
def generate_training_set_many_timesteps(training_set_size, vector_dims, H, t, t_total):
    print("##################################################")
    print("####### Generating training set - (many timesteps)")
    print("##################################################")
    if vector_dims != np.shape(H)[0]:
    ...
    for i in range(training_set_size):
        if i % 100 == 0:
            print("[", i, " / ", training_set_size, "]", end="\r")
        psi_before = generate_random_vector(vector_dims)
        for j in np.arange(t, t_total, t):
            psi_after = U.dot(psi_before)
            input_vectors.append(psi_before)
            output_vectors.append(psi_after)
