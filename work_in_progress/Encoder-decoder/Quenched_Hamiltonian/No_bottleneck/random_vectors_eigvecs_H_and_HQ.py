...
for N in range(N_min, N_max+1):
    ...
    print("#################################")
    legend.append(str(N))
    H = generators.generate_Heisenberg_hamiltonian(N, J=1.0, h=1.0, periodic=False)
    H_quenched = generators.generate_Heisenberg_hamiltonian(N, J=1.0, h=0.0, periodic=False)
    ...
    (input_eigenvectors_H, output_eigenvectors_H) = generators.evolve_whole_set_many_evolution_steps(eigenvectors_H, U_t_quenched, t, t_total)
    eigenvectors_H_train_set_size = floor(np.shape(input_eigenvectors_H)[0]*3/4)

    training_input_eigenvectors_H = input_eigenvectors_H[:eigenvectors_H_train_set_size]
    training_output_eigenvectors_H = output_eigenvectors_H[:eigenvectors_H_train_set_size]

    test_input_eigenvectors_H = input_eigenvectors_H[eigenvectors_H_train_set_size:]
    test_output_eigenvectors_H = output_eigenvectors_H[eigenvectors_H_train_set_size:]

    # Generate eigenvectors of H_quenched, that will go to the training and testing sets
    ...
    (input_eigenvectors_HQ, output_eigenvectors_HQ) = generators.evolve_whole_set_many_evolution_steps(eigenvectors_HQ, U_t_quenched, t, t_total)
    eigenvectors_HQ_train_set_size = floor(np.shape(input_eigenvectors_HQ)[0]*3/4)

    training_input_eigenvectors_HQ = input_eigenvectors_HQ[:eigenvectors_HQ_train_set_size]
    training_output_eigenvectors_HQ = output_eigenvectors_HQ[:eigenvectors_HQ_train_set_size]

    test_input_eigenvectors_HQ = input_eigenvectors_HQ[eigenvectors_HQ_train_set_size:]
    test_output_eigenvectors_HQ = output_eigenvectors_HQ[eigenvectors_HQ_train_set_size:]

    # Concatenate three sets of each type creating as a result training and testing sets
    training_input_vectors = np.concatenate((training_input_vectors, training_input_eigenvectors_H))
    training_input_vectors = np.concatenate((training_input_vectors, training_input_eigenvectors_HQ))
    training_output_vectors = np.concatenate((training_output_vectors,training_output_eigenvectors_H))
    training_output_vectors = np.concatenate((training_output_vectors,training_output_eigenvectors_HQ))
    test_input_vectors = np.concatenate((test_input_vectors, test_input_eigenvectors_H))
    test_input_vectors = np.concatenate((test_input_vectors, test_input_eigenvectors_HQ))
    test_output_vectors = np.concatenate((test_output_vectors, test_output_eigenvectors_H))
    test_output_vectors = np.concatenate((test_output_vectors, test_output_eigenvectors_HQ))
    ...
    #
    dirName = "quenched_H_rand_H_HQ_" + str(train_num_samples) + "_timestep=" + str(t) + "_t_total=" + str(t_total)  + "_optimizer=" + optimizer + "_loss=" + loss + "_activation=" + activation
