...
from ..tools.misc import normalize, quantum_norm
...
def my_norm(x):
    x = tanh(x)
    x = nsl.lib.normalize(x, nsl.configs.NormType.L2, epsilon=1e-09)
    # x = tf.map_fn(normalize, x)
    return x

def train_input_recreation_simple(activation, N, dt, t_max):
    ...
    if path.exists(best_model_filename):
        model = load_model(best_model_filename, custom_objects={"l2_normalize": l2_normalize, "my_cost": my_cost, "my_norm": my_norm})
        print("\n###############################")
        ...
        hidden_layer_0 = Dense(2*(2**N), activation=my_norm)(input)
        # output = Dense(2*(2**N), activation=my_norm)(hidden_layer_0)
        output = Dense(2*(2**N), activation=my_norm)(hidden_layer_0)
