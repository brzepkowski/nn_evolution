...
from misc import overlap_of_reshaped_sets, correlation_in_reshaped_set
...
# reshaped_training_input_vectors = load_array_from_file_2("training_input.dat", 2**(N+1)).astype(np.float64)
# reshaped_training_output_vectors = load_array_from_file_2("training_output.dat", 2**(N+1)).astype(np.float64)
# reshaped_validation_input_vectors = load_array_from_file_2("validation_input.dat", 2**(N+1)).astype(np.float64)
# reshaped_validation_output_vectors = load_array_from_file_2("validation_output.dat", 2**(N+1)).astype(np.float64)
reshaped_test_input_vectors = load_array_from_file_2("testing_input.dat", 2**(N+1)).astype(np.float64)
reshaped_test_output_vectors = load_array_from_file_2("testing_output.dat", 2**(N+1)).astype(np.float64)
test_eigenvalues = load_vector_from_file("eigenvalues_H.dat", 2**N).astype(np.float64) # Load eigenvalues of H
nn_predicted_vectors = load_array_from_file_2("testing_input.dat", 2**(N+1)).astype(np.float64) # Load data for making predictions
...
for time_step in x:
    ...
    # overlaps_simple_op = overlap_of_reshaped_sets(evolved_test_vectors_full_op_reshaped, evolved_test_vectors_simple_op_reshaped)
