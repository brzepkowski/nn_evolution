...
if __name__ == "__main__":
    ...
    print("eigenvalues_H: ", eigenvalues_H)
    print("eigenvalues_HQ: ", eigenvalues_HQ)
    ...
    if not path.exists(HQ_filename):
        print("Hamilotnian HQ hasn't been generated yet. Please run the Fortran program 'generate_data_after_quench' with appropriate arguments")
        sys.exit()
    else:
    ...
    # print("eigenvectors_H: ", eigenvectors_H)
    # print("HQ: ", HQ)

    # exact_energies = energies(eigenvectors_H, HQ)
    # exact_energies.sort()
    # print("EXACT ENERGIES: ", exact_energies)

    spinSite = SpinSite(S=0.5, conserve=None)
    lattice = Chain(L=N, site=spinSite)
    model = HeisenbergModel(lattice=lattice, J=J, h=1.0)
