...
def quantum_norm(vector):
    norm = 0
    for i in range(0, vector.shape[0], 2):
        real_part = vector[i]
        imag_part = vector[i+1]
        norm += real_part**2 + imag_part**2
    return norm

def normalize(vector):
    # tf.print("[BEFORE] vector: ", vector, output_stream=sys.stdout)
    norm = quantum_norm(vector)
    # tf.print("[BEFORE] norm: ", norm, output_stream=sys.stdout)
    vector = tf.math.divide(vector, tf.math.sqrt(norm))
    # tf.print("[AFTER] vector: ", vector, output_stream=sys.stdout)
    # norm = quantum_norm(vector)
    # tf.print("[AFTER] norm: ", norm, output_stream=sys.stdout)
    return vector
